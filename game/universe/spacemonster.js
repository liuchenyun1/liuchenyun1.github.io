...
if($("#alloptions").find("#sm_options").length > 0){
	...
	$('head').append('<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-slider/7.0.0/css/bootstrap-slider.css" type="text/css" />');
	$.getScript('https://cdnjs.cloudflare.com/ajax/libs/bootstrap-slider/7.0.0/bootstrap-slider.js',function(){
		Load_SM();
	});
...
function Load_SM(){
	...
	$("#spacemonster_style").text(""
		...
		+ ".littlebuttons>li>button { height:19px; width:50px; margin-right:1px; outline:none !important; }"
...
function Do_Income(){
	if (atomepersecond > 0) {
		var amount = ((atomepersecond * prestige) * (_timer['income'] / (1000/gameSpeed)));
		totalAtome += amount;
		atomsaccu += amount;
		actualiser();
		checkspec();
	...
	$('#TableObjects').children().each(function(i){
		...
		if($(this).find('ul.littlebuttons').length < 1){
			...
			$(this).find('.divinfo div').append(''
				+ '<div class="sm_unit_type" style="color:' + unit_type[1] + ';" style="display:none;">' + cntype(unit_type[0]) + '</div>'
				+ '<ul class="list-unstyled list-inline text-center littlebuttons">'
					...
					+ '<li><button id="' + id + '_next" class="btn btn-default btn-xs" type="button" onmouseover="seeUnit(' + id + ')" onclick="tonext(' + id + '); Check_Unit_Options();" style="display:none;">下一个</button></li>'
					+ '<li><button id="' + id + '_max" class="btn btn-default btn-xs" type="button" onmouseover="seeUnit(' + id + ')" onclick="tomax(' + id + '); Check_Unit_Options();" style="display:none;">最大</button></li>'
				+ '</ul>'
			+ '');
		}
		var id = $(this).attr('id').replace('tc','') - 1;
...
function Setup_Nav(){
	$(".navig").children("ul").append(""
		+ "<li class id='sm_option'><a class='toclick'><i class='fa fa-rocket'></i>&nbsp;太空怪物</a></li>"
	+ "");
	...
	$("#Main").append("<div id='sm_options'>"
		+ "<div class='row'>"
			+ "<div class='col-sm-12' style=''>"
				+ "<p class='optionsheader' style='color:#88ff88;'>太空怪物 <small style='color:#ffffff; font-size:50%; opacity:0.5;'>"+_SM_Version+"</small></p>"
			+ "</div>"
		+ "</div>"
		+ "<div class='row'>"
			+ "<div class='col-sm-6' style='font-size:20px;'>"
				+ "<center><h1><i class='fa fa-cube'></i> 单位</h1></center>"
				+ "<div class='col-sm-6' align=right>显示单位类型:</div>"
				+ "<div class='col-sm-6' align=left>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_0' value='0' checked> <span>是</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_0' value='1'> <span>否</span>"
					+ "</label>"
				+ "</div>"
				+ "<div class='col-sm-6' align=right>颜色单位按类型:</div>"
				+ "<div class='col-sm-6' align=left>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_1' value='0' checked> <span>是</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_1' value='1'> <span>否</span>"
					+ "</label>"
				+ "</div>"
				+ "<div class='col-sm-6' align=right>禁用买不起的按钮:</div>"
				+ "<div class='col-sm-6' align=left>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_2' value='0' checked> <span>是</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_2' value='1'> <span>否</span>"
					+ "</label>"
				+ "</div>"
				+ "<div class='col-sm-6' align=right>解锁下一个单位:</div>"
				+ "<div class='col-sm-6' align=left>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_10' value='0' checked> <span>是</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_10' value='1'> <span>否</span>"
					+ "</label>"
				+ "</div>"
				+ "<div class='col-sm-6' align=right>显示购买按钮:</div>"
				+ "<div class='col-sm-6' align=left>"
					...
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='checkbox-inline'>"
						+ "<input type='checkbox' name='SM_Option_8' checked> <span>下一个</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='checkbox-inline'>"
						+ "<input type='checkbox' name='SM_Option_9' checked> <span>全部</span>"
					+ "</label>"
				+ "</div>"
			+ "</div>"
			+ "<div class='col-sm-6' style='font-size:20px;'>"
			+ "<center><h1><i class='fa fa-asterisk'></i> 杂项</h1></center>"
				+ "<div class='col-sm-6' align=right>游戏更新率:</div>"
				+ "<div class='col-sm-6' align=left>"
				...
				+ "</div>"
				+ "<div class='col-sm-6' align=right>太空怪兽更新率:</div>"
				+ "<div class='col-sm-6' align=left>"
				...
				+ "</div>"
				+ "<div class='col-sm-12' align=center><small class='sm_note sm_note-danger'><i class='fa fa-exclamation-triangle'></i> 更新率可能会导致游戏运行速度比平时慢 <i class='fa fa-exclamation-triangle'></i></small></div>"
				+ "<div class='col-sm-6' align=right>自动保存游戏频率:</div>"
				+ "<div class='col-sm-6' align=left>"
			...
			+ "</div>"
			+ "<div class='col-sm-6' style='font-size:20px;'>"
			+ "<center><h1><i class='fa fa-desktop'></i> 普通显示</h1></center>"
				+ "<div class='col-sm-6' align=right>滚动到顶部原子条:</div>"
				+ "<div class='col-sm-6' align=left>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_13' value='0' checked> <span>是</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_13' value='1'> <span>否</span>"
					+ "</label>"
				+ "</div>"
				+ "<div class='col-sm-6' align=right>通知位置:</div>"
				+ "<div class='col-sm-6' align=left>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_14' value='0'> <span>左上</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_14' value='1'> <span>右上</span>"
					+ "</label></br>"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_14' value='2'> <span>左下</span>"
					+ "</label>&nbsp;&nbsp;&nbsp;&nbsp;"
					+ "<label class='radio-inline'>"
						+ "<input type='radio' name='SM_Option_14' value='3' checked> <span>右下</span>"
					+ "</label>"
				+ "</div>"
			+ "</div>"
		+ "</div>"
...
function To_Rate(s){
	var out = "";
	switch(s){
		case 1: out = "1 次每 10 秒"; break;
		case 2: out = "1 次每 5 秒"; break;
		case 3: out = "1 次每 3 秒"; break;
		case 4: out = "1 次每 1 秒"; break;
		case 5: out = "2 次每 1 秒"; break;
		case 6: out = "4 次每 1 秒"; break;
		case 7: out = "10 次每 1 秒"; break;
		case 8: out = "15 次每 1 秒"; break;
		case 9: out = "30 次每 1 秒"; break;
		case 10: out = "60 次每 1 秒"; break;
	}
	return out;
}
